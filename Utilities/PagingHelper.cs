...
namespace WebNails.Admin.Utilities
{
    public class PagingHelper
    {
        public static int CountSort
        {
            get
            {
                if (ConfigurationManager.AppSettings["CountSort"] != null)
                {
                    var strResult = ConfigurationManager.AppSettings["CountSort"];
        ...
        public static string Page(int totalRecord, int countPageShow, int countRecordShow, string className)
        {
            ...
            if (maxPage > 1)
            {
                ...
                if (pageIndex > 1)
                {
                    rawHtml = rawHtml + string.Format("<li><a href='{0}{1}page={2}'>«</a><li>", pathQuery, charracter, 1);
                    rawHtml = rawHtml + string.Format("<li><a href='{0}{1}page={2}'>←</a><li>", pathQuery, charracter, previous);
                }
                ...
                {
                    rawHtml = rawHtml + string.Format("<li><a href='{0}{1}page={2}'>→</a><li>", pathQuery, charracter, next);
